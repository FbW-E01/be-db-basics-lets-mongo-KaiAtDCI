...
4. So the (old) mongo shell is kind of like a JavaScript REPL. What is a REPL? Which other REPL have we used?  
    ``
    a) A read–eval–print loop (REPL), also termed an interactive toplevel or language shell, is a simple interactive computer programming environment that takes single user inputs, executes them, and returns the result to the user; a program written in a REPL environment is executed piecewise.
    ``
...
6. (Research) So the old shell is pretty cool. How is the new shell better?  
   ``It is allowing mongodb specific commands.``
...
``  
``Example: db.cats.insertMany([{name:"Veera"}, {name:"Rauli"}]); ``  
...
``
   db.cats.update( {}, { $set: {"cute": true} }, false, true )
``
...
+ Delete collection
  ...
  ```
  ```sql
  db.products.insertOne({name: 'productName'})
  ```
...
+ Read all documents within collection
  ```sql
  db.users.find();
  ```
...
+ Update
  ...
  ```sql
  db.users.updateOne(
  	{_id: ObjectId("6142fc0827714cd6215e9326")}, 
  	{$set: {name: "Karl"}}
  );
  ```
